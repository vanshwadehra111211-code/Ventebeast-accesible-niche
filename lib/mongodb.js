...
if (useFirebase) {
  ...
  // Delay MongoDB connection until the route is actually invoked.
  // This prevents build-time failures when Vercel is collecting page data.
...
export async function getDb() {
  ...
  const uri = process.env.MONGO_URL;
  if (!uri) {
    throw new Error('MONGO_URL is not configured. Set MONGO_URL in your environment for MongoDB backend.');
  }
  if (!clientPromise) {
    client = new MongoClient(uri);
